...
STREAM_CONNECT_TIMEOUT_MS=12000
STREAM_TEST_TIMEOUT_MS=9000
ONVIF_DISCOVER_TIMEOUT_MS=3500
ONVIF_DISCOVER_MAX_RESULTS=30
```

> 说明：为了兼容 RTSP/HLS/HTTP 摄像头，后端会调用 `ffmpeg` 把视频流转成浏览器可播放的 MJPEG，请确保服务器已安装 `ffmpeg` 并可在命令行执行。
